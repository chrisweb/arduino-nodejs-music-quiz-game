...
export class ArduinoLibrary {
    ...
    public lockPlayer(playerId: number, isLock = true) {
      if (this._arduinoSequence != undefined) {
        const index = (playerId * 3);
    ...
    public selectPlayer(playerId: number, isSelected = true) {
      if (this._arduinoSequence != undefined) {
        const index = (playerId * 3) + 2;
    ...
    public sendUpdateStatusButtons() {
      ...
      if (this._port !== null) {
        this._port.write(this._arduinoSequence, (err, results) => {
          console.log(`err: ${err}`);
