...
export class PlayerVisualizer {
  /**
    ...
    protected _transformToVisualBins(initialArray: any) {
      const newArray = new Uint8Array(this._spectrumBarCount);

      for (let i = 0; i < this._spectrumBarCount; i++) {
        const bin = Math.pow(i / this._spectrumBarCount, this._spectrumScale) * (this._spectrumEnd - this._spectrumStart) + this._spectrumStart;
    ...
    protected _savitskyGolaySmooth(binsArray: any) {
      const smoothingPoints = 3; // points to use for algorithmic smoothing. Must be an odd number.
      const smoothingPasses = 1; // number of smoothing passes to execute
    ...
    protected _averageTransform(binsArray: any) {
      ...
      for (let i = 0; i < length; i++) {
        ...
        if (i == 0) {
          ...
          const prevValue = binsArray[i - 1];
          const curValue = binsArray[i];
          const nextValue = binsArray[i + 1];
      ...
      for (let i = 0; i < length; i++) {
        ...
        if (i == 0) {
          ...
          const prevValue = transformedValues[i - 1];
          const curValue = transformedValues[i];
          const nextValue = transformedValues[i + 1];
    ...
    protected _exponentialTransform(binsArray: any) {
      ...
      for (let i = 0; i < binsArray.length; i++) {
        const exp = (this._spectrumMaxExponent - this._spectrumMinExponent) * (1 - Math.pow(i / this._spectrumBarCount, this._spectrumExponentScale)) + this._spectrumMinExponent;
    ...
    protected _updateSizes() {
      ...
      const spectrumWidthDifference = this._spectrumWidth - ((this._barWidth * this._spectrumBarCount) + (this._barSeperation * this._spectrumBarCount));
    ...
    public looper() {
      ...
      // visualizer
      const initialArray = new Uint8Array(this._visualizerAudioGraph.analyserNode.frequencyBinCount);

      this._visualizerAudioGraph.analyserNode.getByteFrequencyData(initialArray);

      const visualBins = this._transformToVisualBins(initialArray);

      const transformedVisualBins = this._getTransformedSpectrum(visualBins);
      ...
      const linearGradient = this._canvasContext.createLinearGradient(0, 0, this._spectrumWidth, 0);

      // color gradiant for the bars
      barsColors.forEach((currentValue, index, array) => {
        // color stop offset, must be between 0 and 1
        const colorStopOffset = ((100 / (array.length - 1)) * (index)) / 100;
      ...
      for (let y = 0; y < this._spectrumBarCount; y++) {
        // dimensions
        const barVerticalPosition = Math.floor(this._sizeDifference / 2) + (y * (this._barWidth + this._barSeperation));
        const barWidth = this._barWidth;
        const barHeight = transformedVisualBins[y];

        const halfBarHeight = Math.floor(barHeight / 2);
        const halfCanvasElementHeight = Math.floor(this._canvasElement.height() / 2);
        const oneThirdOfBarWidth = Math.floor(barWidth / 3);

        // both bars options
        let barCornerRadius = oneThirdOfBarWidth;
        const barFill = true;
        const barStroke = false;
    ...
    protected _roundRect(canvasContext: CanvasRenderingContext2D, topLeftX: number, topLeftY: number, rectangleWidth: number, rectangleHeight: number, cornerRadius: number | ICornerRadius, fill: boolean, stroke: boolean) {
      ...
      if (typeof cornerRadius === 'number') {
        ...
        const defaultRadius: ICornerRadius = {
